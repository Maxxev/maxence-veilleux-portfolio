...
faits:
  ...
  - 'A preview section and its full page are always fed by the same source and the same grid component, so they cannot drift apart over time.'
...
It's a constraint you accept up front and that pays off on every later change.
...
This portfolio is built on exactly that foundation. The dark mode you can
toggle at the top of this page required no theme-specific rule in any
component: only one layer of semantic tokens had to be redefined.
